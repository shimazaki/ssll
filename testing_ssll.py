...
def klic(p_theta, q_theta, N):
    ...
    for i in range(T):
        ...
        log_prob_q = numpy.dot(q_theta[i,:], transforms.eta_map.toarray()) - phi_q
        log_prob_p = numpy.dot(p_theta[i,:], transforms.eta_map.toarray()) - phi_p
...
class TestEstimator(unittest.TestCase):
    ...
    def test_fo_varying(self):
        ...
        for N in 2**numpy.arange(4):
            ...
            cells = random.sample(list(numpy.arange(N)), n_random)
    ...
    def test_so_variable(self):
        ...
        for N in 2**numpy.arange(1, 4):
            ...
            interactions = random.sample(list(numpy.arange(N, D)), n_random)
            for i in range(n_random):
                # Draw random phase, amplitude and frequency
                phi = numpy.random.uniform(0, 2 * numpy.pi)

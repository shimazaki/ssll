...
def generate_thetas(N, O, T, mu1=-2., sigma1=50, mu2=0., sigma2=50, alpha=12., ratio_modulated=1., seed=None):
    ...
    :param float ratio_modulated:
        Scalar between 0 and 1, how many of the interaction should be modulated.
        (Default=1)
    :param int seed:
    ...
    theta[:,:N] = mu1 + numpy.dot(L1, rng.standard_normal((T, N)))
    theta[:,N:] = mu2 + numpy.dot(L2, rng.standard_normal((T, D - N)))
    num_non_modulated = int(numpy.around((1. - ratio_modulated)*(D - N)))
    # Use rng.choice to select indices and ensure they are integers
    non_modulated_idx = rng.choice(numpy.arange(N, D, dtype=numpy.int_),
                                   size=num_non_modulated,
                                   replace=False)
...
def generate_stationary_thetas(N, O, T, seed=None):
    ...
    th = numpy.zeros([T,D])
    th[:,:N] = th1
    th[:,N:] = th2/N*(1 + 0.5*rng.standard_normal((T,D-N)))
...
def generate_spikes(p, R, seed=None):
    ...
    """
    # Set metadata
    T, N = p.shape[0], int(numpy.log2(p.shape[1]))

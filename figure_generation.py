...
def figure4(data_path='../Data/'):
    ...
    for i in range(num_of_networks):
        ...
        spikes = synthesis.generate_spikes_gibbs_parallel(theta_all, (i + 1) * N, O, R, sample_steps=10)
        emd = __init__.run(spikes, O, map_function='cg', param_est='pseudo', param_est_eta='bethe_hybrid',
                           lmbda1=100,
                           lmbda2=200)
        ...
        for t in range(T):
            eta_est[t], psi_est[t] = bethe_approximation.compute_eta_hybrid(emd.theta_s[t], (i + 1) * N,
                                                                            return_psi=1)
            psi1 = bethe_approximation.compute_eta_hybrid(.999 * emd.theta_s[t], (i + 1) * N, return_psi=1)[1]
            psi2 = bethe_approximation.compute_eta_hybrid(1.001 * emd.theta_s[t], (i + 1) * N, return_psi=1)[1]
            S_est[t] = -(numpy.sum(eta_est[t] * emd.theta_s[t]) - psi_est[t])
        ...
        f = h5py.File(data_path + 'figure4data.h5', 'r+')
        f['error500']['MISE_thetas'][i] = numpy.mean((theta_all - emd.theta_s) ** 2)
        f['error500']['MISE_population_rate'][i] = numpy.mean((population_rate - population_rate_est) ** 2)

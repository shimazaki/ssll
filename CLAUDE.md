...
2. **`container.py`** (`EMData`) — Holds all EM state: natural parameters (theta), covariances (sigma), expectation parameters (eta), hyperparameters (Q, F, U). Functions modify EMData in-place.
3. **`exp_max.py`** — E-step (forward filter + backward smoother) and M-step (hyperparameter optimization for Q, F, U). E-step calls MAP estimation to find posterior mode at each timestep.
...
- `u`: Exogenous input array (T, n_u) or None. When provided, the state equation becomes θ_t = F·θ_{t-1} + U·u_t + ξ_t; U (D×n_u) is learned via `m_step_U` in `exp_max.py`.
